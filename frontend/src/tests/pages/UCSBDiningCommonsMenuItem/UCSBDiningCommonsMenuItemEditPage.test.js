...
jest.mock('react-toastify', () => {
    ...
    return {
        __esModule: true, ...originalModule, toast: (x) => mockToast(x)
    };
});
...
jest.mock('react-router-dom', () => {
    ...
    return {
        __esModule: true, ...originalModule, useParams: () => ({
            id: 17
        }), Navigate: (x) => {
            mockNavigate(x);
            return null;
    ...
    describe("tests where backend is working normally", () => {
        ...
        beforeEach(() => {
            ...
            axiosMock.onGet("/api/ucsbdiningcommonsmenuitem", {params: {id: 17}}).reply(200, {
                id: 17, diningCommonsCode: "ortega", name: "Baked Pesto Pasta with Chicken", station: "Entree Specials"
            });
            axiosMock.onPut('/api/ucsbdiningcommonsmenuitem').reply(200, {
                id: "17", diningCommonsCode: "ortega", name: "Tofu Banh Mi Sandwich (v)", station: "Entree Specials"
            });
        });
        ...
        test("Is populated with the data provided", async () => {
            ...
            fireEvent.change(nameField, {target: {value: 'Tofu Banh Mi Sandwich (v)'}});
            fireEvent.change(stationField, {target: {value: 'Entree Specials'}});
            fireEvent.click(submitButton);

            await waitFor(() => expect(mockToast).toBeCalled());
            expect(mockToast).toBeCalledWith("UCSB Dining Commons Menu Item Updated - id: 17 name: Tofu Banh Mi Sandwich (v)");
            ...
            expect(axiosMock.history.put[0].data).toBe(JSON.stringify({
                diningCommonsCode: 'ortega', name: 'Tofu Banh Mi Sandwich (v)', station: 'Entree Specials'
            })); // posted object
