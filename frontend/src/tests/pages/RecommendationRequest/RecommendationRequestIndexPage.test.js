...
describe("RecommendationRequestIndexPage tests", () => {
    ...
    test("Renders with Create Button for admin user", async () => {
        ...
        const button = screen.getByText(/Create Recommendation/);
        expect(button).toHaveAttribute("href", "/recommendationrequest/create");
        expect(button).toHaveAttribute("style", "float: right;");
    });

    test("renders three reommendations correctly for regular user", async () => {
        
        // arrange
        setupUserOnly();
        const queryClient = new QueryClient();
        axiosMock.onGet("/api/recommendationrequest/all").reply(200, recommendationRequestFixtures.threeRecommendation);
        ...
        expect(screen.queryByText(/Create Recommendation/)).not.toBeInTheDocument();
