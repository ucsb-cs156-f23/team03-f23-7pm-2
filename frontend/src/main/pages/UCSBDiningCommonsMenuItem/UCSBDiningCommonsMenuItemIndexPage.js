...
export default function UCSBDiningCommonsMenuItemIndexPage() {
    ...
    const {
        data: items, error: _error, status: _status
    } = useBackend(// Stryker disable next-line all : don't test internal caching of React Query
        ["/api/ucsbdiningcommonsmenuitem/all"], {method: "GET", url: "/api/ucsbdiningcommonsmenuitem/all"}, []);

    return (<BasicLayout>
            <div className="pt-2">
                {createButton()}
                <h1>UCSB Dining Commons Menu Item</h1>
                <UCSBDiningCommonsMenuItemTable menuItem={items} currentUser={currentUser}/>
            </div>
    </BasicLayout>);
}
